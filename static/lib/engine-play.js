...
export function initEnginePlay({ getPieceSet, getTheme, onSound, onSnapshot }) {
  ...
  if (!replayConfig.browserEngines?.reckless?.enabled) {
    ...
    recklessOption.textContent = "Reckless · coming soon (browser assets not installed)";
  }
  const state = {
    chess: new Chess(),
    engine: null,
  ...
  async function makeAdapter() {
    if (state.engineId === "stockfish") {
      const stockfish = createEngine("stockfish-browser");
      await stockfish.init();
      return {
        bestMove: async fen => (await stockfish.evaluate(fen)).bestmove,
        close: () => stockfish.close(),
      };
    }
    if (state.engineId === "reckless") {
      const config = replayConfig.browserEngines?.reckless || {};
      if (!config.enabled) throw new Error("Reckless is coming soon on this deployment. Its browser dist assets have not been installed yet.");
      const assetBaseUrl = new URL(config.assetBaseUrl || "./vendor/reckless/", document.baseURI).href;
      const module = await import(new URL("reckless-engine.js", assetBaseUrl).href);
      const reckless = new module.RecklessEngine({ assetBaseUrl });
      await reckless.init();
      return {
        bestMove: async fen => (await reckless.analyze({ fen, movetime: 900, multiPv: 1 })).bestMove,
        close: () => reckless.terminate(),
      };
    }
    throw new Error("That engine is not available in this deployment.");
  }
